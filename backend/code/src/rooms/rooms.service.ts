...
export class RoomsService {
  ...
  async getRoomMembers(
    ...
    return filtredmembers.map((member) => {
      if (!member.is_banned || user.is_admin) {
        ...
        const avatar: PICTURE = buildAvatar(member.user.avatar, name);
        return {
  ...
  async seedTestChatData(userId: string) {
    ...
    const testEmail = 'test.user@local.dev';
    ...
    if (!testUser) {
      testUser = await this.prisma.user.create({
        data: {
          ...
          firstName: 'Test',
          lastName: 'User',
          discreption: 'Test account for chat',
    ...
    if (testUser.userId === userId) {
      return {
        seeded: false,
        message: 'test user conflicts with current user',
      };
    }
    ...
    const existingDm = await this.prisma.room.findFirst({
      where: {
        ...
        AND: [
          { members: { some: { userId } } },
          { members: { some: { userId: testUser.userId } } },
        ],
      },
    });
    ...
    if (existingDm) {
      return {
        seeded: false,
        message: 'test dm already exists',
        dmRoomId: existingDm.id,
        testUserId: testUser.userId,
      };
    }

    const dmRoom = await this.prisma.room.create({
      data: {
        type: 'dm',
        owner: {
          connect: { userId },
    ...
    const publicRoom = await this.prisma.room.create({
      data: {
        name: 'Test Room',
        type: 'public',
        owner: {
    ...
    await this.prisma.message.create({
      data: {
        content: 'Hey! This is your test DM.',
        roomId: dmRoom.id,
    ...
    await this.prisma.message.create({
      data: {
        content: 'Nice to meet you. This is a sample reply.',
        roomId: dmRoom.id,
        authorId: userId,
    ...
    await this.prisma.message.create({
      data: {
        content: 'Welcome to the public test room!',
        roomId: publicRoom.id,
        authorId: testUser.userId,
