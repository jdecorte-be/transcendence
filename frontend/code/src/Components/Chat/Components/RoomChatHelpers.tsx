...
export const InitChatPlaceholder = () => {
  ...
  const canSeed =
    chatState.recentDms.length === 0 && chatState.recentRooms.length === 0;

  const handleSeedTestData = async () => {
    if (!canSeed || isSeeding) return;
    setIsSeeding(true);
    try {
      const seedRes = await seedTestChatDataCall();
      if (seedRes?.status !== 200 && seedRes?.status !== 201) return;
      if (seedRes?.data?.seeded === false) {
        toast.error(seedRes.data.message ?? "Seed skipped");
        return;
      }

      const [dmsRes, roomsRes] = await Promise.all([
        fetchDmsCall(0, 7),
        fetchRoomsCall(0, 7, true),
      ]);

      if (dmsRes?.status === 200 || dmsRes?.status === 201) {
        const rooms: DmRoom[] = [];
        dmsRes.data.forEach(
          (room: {
        ...
        chatState.fillRecentDms(rooms);
      }

      if (roomsRes?.status === 200 || roomsRes?.status === 201) {
        const rooms: ChatRoom[] = [];
        roomsRes.data.forEach(
          (room: {
            countMembers: number;
            id: string;
            is_admin: boolean;
        ...
        chatState.fillRecentRooms(rooms);
      }

      chatState.changeChatType(ChatType.Chat);
      toast.success("Test user and rooms added");
    } finally {
      setIsSeeding(false);
    }
  };
    ...
    <div className="null image flex flex-col justify-center items-center h-full">
      ...
      {canSeed && (
        <button
          ...
          disabled={isSeeding}
        >
          {isSeeding ? "Adding test data..." : "Add test user and rooms"}
        </button>
      )}
    </div>
  );
