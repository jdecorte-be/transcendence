...
export const Profile = ({ selected, className, ...props }: ProfileProps) => {
  ...
  return (
    <Link to={`Profile/${userStore?.id}`}>
      ...
      >
        <div
          className={`h-9 w-9 rounded-xl flex justify-center items-center hover:cursor-pointer hover:bg-primary/10 transition-all duration-200 hover:scale-110 active:scale-95 ${
            selected ? "bg-primary/15 scale-105" : ""
          }`}
        >
          <svg
            width="19"
